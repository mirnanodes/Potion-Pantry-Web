...
if (!$product_id) {
    redirect('dashboard.php', 'Invalid product ID', 'error');
}

// Check if user owns this product
if (!userOwnsProduct($_SESSION['user_id'], $product_id)) {
    redirect('dashboard.php', 'Product not found or access denied', 'error');
}

// Get product details
$stmt = getDB()->prepare("SELECT * FROM Products WHERE product_id = ? AND user_id = ?");
$stmt->execute([$product_id, $_SESSION['user_id']]);
$product = $stmt->fetch();

if (!$product) {
    redirect('dashboard.php', 'Product not found', 'error');
}

// Get all ingredients
$ingredients = getAllIngredients();

// Get current product ingredients
$stmt = getDB()->prepare("SELECT ingredient_id FROM ProductIngredients WHERE product_id = ?");
$stmt->execute([$product_id]);
$current_ingredients = $stmt->fetchAll(PDO::FETCH_COLUMN);
    ...
    if (empty($product_name) || empty($brand) || empty($product_type) || 
        ...
        try {
            ...
            if (empty($error)) {
                ...
                $stmt = $db->prepare("
                    UPDATE Products 
                    SET product_name = ?, brand = ?, product_type = ?, purchase_date = ?, 
                        expiration_date = ?, price = ?, volume_ml = ?, notes = ?, image_path = ?
                    WHERE product_id = ? AND user_id = ?
                ");
                $stmt->execute([
                    $product_name, $brand, $product_type, $purchase_date,
                    $expiration_date, $price, $volume_ml, $notes, $image_path,
                ...
                $stmt = $db->prepare("DELETE FROM ProductIngredients WHERE product_id = ?");
                $stmt->execute([$product_id]);
                ...
                $db->commit();
                redirect('dashboard.php', 'Product updated successfully!');
            }
        } catch (Exception $e) {
            $db->rollBack();
...
<head>
    ...
    <title>Edit Product - Potion Pantry</title>
                        ...
                        <div class="card shadow">
                            <div class="card-body p-4">
                                <div class="text-center mb-4">
                                    ...
                                    <p class="text-muted">Update your skincare product information</p>
                                </div>
                                ...
                                <form method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <select class="form-select" id="product_type" name="product_type" required>
                                                ...
                                                <?php
                                                $types = ['Cleanser', 'Toner', 'Serum', 'Moisturizer', 'Sunscreen', 'Treatment', 'Mask', 'Eye Care'];
                                                foreach ($types as $type):
                                                ?>
                                                    <option value="<?= $type ?>" <?= $product['product_type'] === $type ? 'selected' : '' ?>>
                                                        <?= $type ?>
                                                    </option>
                                                <?php endforeach; ?>
                                            </select>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control" id="notes" name="notes" rows="3"><?= htmlspecialchars($product['notes']) ?></textarea>
                                    </div>
